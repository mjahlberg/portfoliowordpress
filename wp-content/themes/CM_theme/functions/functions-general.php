...
if(!function_exists('pexeto_get_header_bg_data')){
	...
	function pexeto_get_header_bg_data(){
		...
		if(!empty($bg_options['img']) && !$contains_slider){
			...
			$res['img'] = sprintf('<div data-stellar-background-ratio="0.2" class="full-bg-image" style="background-image:url(%s); opacity:%s; filter: alpha(opacity=%d);"></div>', $bg_options['img'], $bg_options['opacity'], $ie_opacity);
		}
